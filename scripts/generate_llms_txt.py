...
def _render(tutorials: list[tuple[str, str, dict]]) -> str:
    lines: list[str] = [
        ...
        ">",
        "> Compose a model, dataset, and recipe (`SlimeRecipe` / Miles) via",
        "> `TrainConfig`, then call `.train()` / `.launch()` — cluster topology,",
        "> Ray/NCCL, volumes, and checkpointing are handled.",
        "",
