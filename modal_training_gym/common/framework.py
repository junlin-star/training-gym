...
# image. Framework-agnostic helpers (e.g. tokenizer preparation) shell out to
# scripts at this path, so it must exist on every launcher's image.
...
def mount_tools_dir(image: "Image") -> "Image":
    ...
    cross-framework scripts (e.g. `ensure_glm_tokenizer.py`) resolve at a
    predictable remote path regardless of the framework image's layout.
    """
