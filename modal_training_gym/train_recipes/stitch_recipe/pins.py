...
# Vendored from stitch's ``cookbook/miles_disagg/patches``. They are applied to the
# Megatron source tree at container start (not at build time) so an image layer is
# shared across recipes that need different subsets. R3 routing replay needs the
# dropless-dispatch fix; the reshardable-step patch lets a CPU-offloaded optimizer
# resume across a changed DP layout.
MEGATRON_PATCH_DIR = "/root/megatron-patches"
