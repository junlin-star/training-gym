...
def _agentic_metrics(samples) -> dict:
    ...
    out["agentic/format_error_tax/mean"] = mean("n_calls") - mean(
        "turns"
    )  # non-productive calls per episode
    for key in (
        "turns",
        "response_tokens",
        "output_tokens",
        ...
        out[f"agentic/{key}/max"] = float(
            max((s.get(key, 0) for s in stats), default=0)
        )
    turns_list = [
        s["turns"] for s in stats if "turns" in s
    ]  # turn distribution (decide max_steps)
    for p in (50, 90, 99):
        out[f"agentic/turns/p{p}"] = (
            float(np.percentile(turns_list, p)) if turns_list else 0.0
        )
    ...
    out["agentic/truncated_frac"] = (
        sum(s.get("exit_status") in _trunc_exits for s in stats) / n
    )
    out["agentic/truncated/step_cap_frac"] = mean(
        "hit_step_cap"
    )  # turns: hit max_steps
    out["agentic/truncated/context_frac"] = (
        sum(s.get("exit_status") == "ContextExceeded" for s in stats) / n
    )
    out["agentic/truncated/time_frac"] = (
        sum(s.get("exit_status") == "TimeExceeded" for s in stats) / n
    )
    out["agentic/truncated/length_turn_frac"] = (
        sum(s.get("length_truncations", 0) > 0 for s in stats) / n
    )
    gens = [
        (s["output_tokens"], s["gen_time"])
        for s in stats
        if s.get("gen_time", 0) > 0 and "output_tokens" in s
    ]
    if gens:
        out["agentic/decode_tok_per_s/mean"] = float(np.mean([o / g for o, g in gens]))
    frac = [
        s["gen_time"] / s["episode_time"]
        for s in stats
        if s.get("episode_time", 0) > 0 and "gen_time" in s
    ]
    if frac:
    ...
    for status, c in exits.items():
        ...
        grp = [
            s.get("solved", 0.0) for s in stats if s.get("exit_status", "?") == status
        ]
        out[f"agentic/solved_by_exit/{status}"] = float(np.mean(grp)) if grp else 0.0
    rsrc = Counter(
        s.get("reward_source", "?") for s in stats
    )  # curated submission vs git-add-A fallback
...
def _async_metrics(samples, now) -> dict:
    out = {}
    versioned = [
        s.weight_versions for s in samples if getattr(s, "weight_versions", None)
    ]
    if versioned:
    ...
    try:
        ...
        if samples:
            extra["agentic/removed_frac"] = float(
                np.mean(
                    [
                        1.0 if getattr(sample, "remove_sample", False) else 0.0
