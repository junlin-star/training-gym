"""Ray cluster bring-up helpers for multi-node disaggregated training.

Vendored from the stitch ``slime_disagg`` cookbook (originally adapted from
...
def training_nodes(cfg: Any) -> int:
    ...
    if (
        getattr(cfg, "use_critic", False)
        or getattr(cfg, "advantage_estimator", None) == "ppo"
    ):
...
def start_ray_head(my_ip: str, n_nodes: int, *, ray_port: int) -> None:
    ...
    try:
        ...
        _print_ray_logs()
        raise RuntimeError(
            f"Ray head node failed to start with exit code {exc.returncode}"
        ) from exc
