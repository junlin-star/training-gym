          ...
          <div class="steps" style:height={`${rowHeight}px`}>
            {#each timeline.steps as step (step.id)}
              <div
                ...
                title={`Step ${step.id}: ${fmtSecs(step.duration)} wall clock — ${fmtSecs(step.work)} work, ${fmtSecs(step.idle)} measured idle`}
              >
...
{#if tip}
  ...
  >
    ...
    {/if}
    {#if showOpenRollout && ["generate_samples", "generate_rollouts"].includes(tip.bar.name) && tip.bar.kind !== "idle" && rolloutIdForBar(tip.bar) != null && rolloutIds.includes(rolloutIdForBar(tip.bar)) && onOpenRollout}
      <button
        class="tg-tip-action"
        onclick={(e) => {
          e.stopPropagation();
