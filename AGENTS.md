...
`dashboards/app.py` is a Modal app with a Svelte frontend (built at image-build time). Training runs write metadata to a shared Modal Volume (`training-gym-metadata`) via `modal_training_gym.utils.metadata`. The ASGI endpoint serves the pre-built SPA and reads summary JSON from the volume on demand.
