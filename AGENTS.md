...
- Served endpoints (`DeploymentConfig.serve()`) sit behind Modal proxy auth by default: export a proxy-auth token pair `MODAL_KEY` (`wk-…`) / `MODAL_SECRET` (`ws-…`) in the launching shell, or every eval/`generate`/teacher call returns HTTP 401. Pass `unauthenticated=True` to `DeploymentConfig` for a public SGLang endpoint (e.g. Modal Notebooks). vLLM rejects `unauthenticated=True` (`TrainingGymConfigError`) because `modal.experimental.http_server` has no proxy-auth knob yet. For calls from remote workers (custom rm/reward fns) to authenticated endpoints, also forward the pair into the worker via a `modal.Secret` — the driver shell env doesn't reach them.
