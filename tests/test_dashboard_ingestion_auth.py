...
def test_timing_capability_requires_run_token(monkeypatch, tmp_path):
    static = tmp_path / "static"
    (static / "assets").mkdir(parents=True)
    (static / "index.html").write_text("ok")
    (static / "favicon.svg").write_text("<svg/>")
...
def test_timing_capability_accepts_the_run_token(fake_volume, monkeypatch, tmp_path):
    _save_run()
    static = tmp_path / "static"
    (static / "assets").mkdir(parents=True)
    (static / "index.html").write_text("ok")
    ...
    assert response.status_code == 200
    assert response.json() == {"protocol": "training-gym-substep-timing"}
