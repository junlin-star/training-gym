...
def test_ld_library_path_comes_from_the_container(monkeypatch):
    """Workers get the container's native linker path."""
    ...
    assert env_vars["LD_LIBRARY_PATH"] == "/usr/local/cuda/lib64:/wheel/nvidia/lib"
...
def test_unset_container_path_does_not_set_an_empty_linker_path(monkeypatch):
...
def test_container_entries_are_not_duplicated(monkeypatch):
